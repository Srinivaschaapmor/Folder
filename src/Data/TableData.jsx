...
function TableData({ handleDelete, value, index, parentKey, handleDownload }) {
  // console.log({ value });
          ...
          <TableRow>
            <TableCell sx={{ p: 1, color: "#393939" }}>
              <Stack direction={"row"} alignItems={"center"} sx={{ pl: 2 }}>
                {getFileIcon(file.name.split(".").slice(-1)[0])}
                <Typography sx={{ width: 200, pl: 2 }}>{file.name}</Typography>
